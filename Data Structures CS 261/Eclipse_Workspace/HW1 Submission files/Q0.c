...
/* Name: Xiao Kuang
 * Date: 1/20/2019
 * Solution description: This program demonstrates passing variables by reference to functions in C.
 */
...
void fooB(int* jptr){
	...
	printf("Value pointed to by jptr: %d\n", *jptr);					//*jptr is dereferencing the pointer
	printf("Address of integer pointed to by jptr: %p\n", jptr);		//jptr is the address of the variable it points to
